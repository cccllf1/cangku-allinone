      ...
      <List
        ...
        renderItem={item => (
          <Collapse
            ...
            style={{ background: '#fff', marginBottom: 2 }}
          >
            <Collapse.Panel
              key={item.key || item.productCode}
              header={
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  <div style={{ marginRight: 16, width: 75, height: 75, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #f0f0f0', borderRadius: 4, overflow: 'hidden' }}>
                    <img
                      src={getFullImageUrl(item.image || '')}
                      alt={item.productName || item.productCode}
                      style={{ width: 75, height: 75, objectFit: 'contain', borderRadius: 4, background: '#f5f5f5' }}
                    />
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 'bold' }}>{item.productName || item.productCode}</div>
                    <div style={{ fontSize: 12, color: '#888' }}>编码: {item.productCode}</div>
                    <Tag color="orange">总库存: {item.quantity} {item.unit || '件'}</Tag>
                    <Tag color="purple">款式: {item.skuList ? item.skuList.length : 0}</Tag>
                    <Tag color="green">库位: {item.locations ? item.locations.length : 0}</Tag>
                  </div>
                </div>
              }
            >
              {/* 展开内容：SKU明细 from item.skuList，改为颜色-尺码二级折叠 */}
              {item.skuList && item.skuList.length > 0 ? (
                Object.entries(groupSkusByColor(item.skuList)).map(([color, skus]) => (
                  <Collapse key={color} bordered={false} style={{ background: '#fafbfc', marginBottom: 8, marginLeft: 8 }}>
                    <Collapse.Panel
                      key={color}
                      header={
                        <div style={{ display: 'flex', alignItems: 'center' }}>
